...
//
// This file is part of the robot code developed by Team 3255 SuperNURDS.
// It is intended for use in the FIRST Robotics Competition.
//
// Original Work by Team 3255 SuperNURDS
...
public class CS_XboxController extends XboxController {
  ...
  public Trigger btn_LeftTrigger =
      new Trigger(() -> super.getLeftTriggerAxis() > TRIGGER_PRESS_THRESHOLD);
  public Trigger btn_RightTrigger =
      new Trigger(() -> super.getRightTriggerAxis() > TRIGGER_PRESS_THRESHOLD);
  ...
  public DoubleSupplier axis_RightX =
      () -> MathUtil.applyDeadband(super.getRightX(), RIGHT_DEADBAND);
  public DoubleSupplier axis_RightY =
      () -> MathUtil.applyDeadband(-super.getRightY(), RIGHT_DEADBAND);
  ...
  /**
   * Set the value that the triggers must be at to register as true while being used as a button. A
   * value of zero will make the trigger act as a hair trigger (will activate at the slightest
   * touch) and a value of one will make the trigger be held all the way down to activate.
   *
   ...
   */
  public CS_XboxController(int port, double leftDeadband, double rightDeadband) {
    super(port);
   ...
   * @param port The port index on the Driver Station that the controller is plugged into.
   */
  public CS_XboxController(int port) {
    super(port);
  }
}
