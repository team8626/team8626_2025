...
public class ToAlgaeAimAndShoot extends SequentialCommandGroup {
  private final Timer timer = new Timer();

  public ToAlgaeAimAndShoot(Supplier<AlgaePreset> algaePresetSupplier) {
    AlgaeShooterSubsystem algae501 = RobotContainer.algae501;

    AlgaePreset algaePreset = algaePresetSupplier.get();

    System.out.println("[Cmd: TOALGAEAIMANDSHOOT]");
    addCommands(
        // new ConditionalCommand(
        new SequentialCommandGroup(
                Commodore.getSetStateCommand(CommodoreState.ALGAE_SHOOT_RAMPINGUP),
                new ParallelCommandGroup(
                    new ElevatorSetHeight(() -> algaePreset.getElevatorHeightInches()),
                    new WristSetAngle(() -> algaePreset.getWristAngleDegrees())),
                Commodore.getSetStateCommand(CommodoreState.ALGAE_SHOOT_RAMPINGUP),
                new AlgaeShooterRampUp(() -> algaePreset.getRPM()) {
                  @Override
